...
void AirTravelSimulator::Run()
{
	...
	cin >> answer;
	m_airport.SetMaxCapacity(answer);
...
{
	int index = rand() % 1000;
	Traveller* ptr = m_pplManager.GetTraveller(index);
	m_airport.LineUp(ptr);
	m_ptrTravellers.PushBack(ptr);
	ptr->state = LINEUP;
	DisplayMessage(ptr, LINEUP);
...
{
	Traveller* ptr = m_airport.NextInLine();
	m_airplane.Board(ptr);
	ptr->state = BOARDING;
	DisplayMessage(ptr, BOARDING);
...
{
	Traveller* ptr = m_airplane.Disembark();
	ptr->state = FINISHED;
	DisplayMessage(ptr, FINISHED);
